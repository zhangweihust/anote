...
public class MainScreen extends TabActivity implements OnTabChangeListener, OnClickListener{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mTabHost.addTab(buildTabSpec(TAB_DYNAMIC, R.drawable.tabhost_dynamic_selector, new Intent(this, DynamicScreen.class)));        
        mTabHost.addTab(buildTabSpec(TAB_HOME, R.drawable.tabhost_home_selector, new Intent(this, HomeScreen.class)));
        mTabHost.addTab(buildTabSpec(TAB_PLAZA, R.drawable.tabhost_plaza_selector, new Intent(this, PlazaScreen.class)));
        mTabHost.addTab(buildTabSpec(TAB_FRIEND, R.drawable.tabhost_friend_selector, new Intent(this, FriendScreen.class)));
        mTabHost.addTab(buildTabSpec(TAB_MORE, R.drawable.tabhost_more_selector, new Intent(this, MoreScreen.class)));   
        mTabHost.setCurrentTab(INIT_SELECT);
        mTabHost.setOnTabChangedListener(this);
        
        mCurSelectTabIndex = INIT_SELECT;
		View currentTab = mTabHost.getCurrentTabView();
		ImageView tabPressed = (ImageView)currentTab.findViewById(R.id.tab_pressed);
		 ...
		 if (tabId.equalsIgnoreCase(TAB_HOME)){
			 if(!type.equals(TYPE_CALENDAR)){
				MainScreen.this.runOnUiThread(new Runnable(){
				 ...
				 mtvTitleBarTitle.setText(MainScreen.this.getResources().getText(R.string.home_screen_calendar_page_title));
			 }
		 }else{
			MainScreen.this.runOnUiThread(new Runnable(){
				@Override
				public void run() {
					...
					mbtnTitleBarNoteAlbum.setVisibility(View.GONE);
					mtvTitleBarTitle.setVisibility(View.VISIBLE);
					mtvTitleBarTitle.setText(MainScreen.this.getResources().getText(R.string.home_screen_title));
					
				}});
		 }
	    View lastTab = mTabHost.getTabWidget().getChildTabViewAt(mCurSelectTabIndex);
		ImageView lastTabPressed = (ImageView)lastTab.findViewById(R.id.tab_pressed);
	...
	public void onClick(View arg0) {
		...
		case R.id.btn_title_bar_calendar:
			...
			HomeScreen.eventService.onUpdateEvent(new EventArgs(EventTypes.TITLE_BAR_CALENDER_CLICKED));
			break;
		...
		case R.id.btn_title_bar_notebook:
			MainScreen.this.runOnUiThread(new Runnable(){
				...
				public void run() {
					...
					mtvTitleBarTitle.setVisibility(View.GONE);	
					mbtnTitleBarNoteAlbum.setVisibility(View.VISIBLE);
					mbtnTitleBarNoteAlbum.setBackgroundResource(R.drawable.title_bar_note);
				}});
			HomeScreen.eventService.onUpdateEvent(new EventArgs(EventTypes.TITLE_BAR_NOTEBOOK_CLICKED));
			break;
		case R.id.btn_title_bar_note_album:
			...
			HomeScreen.eventService.onUpdateEvent(new EventArgs(EventTypes.TITLE_BAR_NOTE_ALBUM_CLICKED).putExtra("type", type));
			break;
		default:
