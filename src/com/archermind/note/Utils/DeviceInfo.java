...
public class DeviceInfo {
	public static String getInformation(InfoName infoName) {
		...
		DisplayMetrics dm = NoteApplication.getContext().getResources().getDisplayMetrics();
		switch (infoName) {
		case IMEI:
		...
		case NOTE_VERSION:
			try {
				PackageInfo packageInfo = NoteApplication.getContext().getPackageManager().getPackageInfo(NoteApplication.getContext().getPackageName(), PackageManager.GET_CONFIGURATIONS);
	...
	public enum InfoName {
		IMEI("imei"), SYSTEM_VERSION("osVersion"), NOTE_VERSION("softVersion"), CPU_MODEL("cpuModel"), CPU_MAX_FREQUENCY("cpuClk"), 
