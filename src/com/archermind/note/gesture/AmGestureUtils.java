...
public class AmGestureUtils {
     ...
     */
    public static float[] spatialSampling(AmGesture gesture, int bitmapSize) {
    ...
    public static float[] spatialSampling(AmGesture gesture, int bitmapSize, 
        ...
        final ArrayList<AmGestureStroke> strokes = gesture.getStrokes();
        final int count = strokes.size();
        int size;
        float xpos;
        float ypos;
        for (int index = 0; index < count; index++) {
            final AmGestureStroke stroke = strokes.get(index);
     ...
     */
    public static float[] temporalSampling(AmGestureStroke stroke, int numPoints) {
     ...
     */
    public static AmOrientedBoundingBox computeOrientedBoundingBox(ArrayList<AmGesturePoint> originalPoints) {
        final int count = originalPoints.size();
        float[] points = new float[count * 2];
        for (int i = 0; i < count; i++) {
            AmGesturePoint point = originalPoints.get(i);
            int index = i * 2;
     ...
     * @return an oriented bounding box
     */
    public static AmOrientedBoundingBox computeOrientedBoundingBox(float[] originalPoints) {
    ...
    private static AmOrientedBoundingBox computeOrientedBoundingBox(float[] points, float[] centroid) {
        ...
        return new AmOrientedBoundingBox((float) (angle * 180 / Math.PI), centroid[0], centroid[1], maxx - minx, maxy - miny);
    }
